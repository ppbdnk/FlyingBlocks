...
{
    return degrees(roll_);
}

double IMUGY85::getPitch()
{
    return degrees(pitch_);
}

double IMUGY85::getYaw()
{
    double indegrees = degrees(yaw_);
...
void IMUGY85::getEuler(double *yaw, double *pitch, double *roll)
{
	*yaw = degrees(yaw_);
	*pitch = degrees(pitch_);
	*roll = degrees(roll_);
}
...
// device orientation -- which can be converted to yaw_, pitch_, and roll_. Useful for stabilizing quadcopters, etc.
